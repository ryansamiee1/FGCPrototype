...
var WebCodeCamJS = function(element) {
    ...
    var display = Q(element),
        ...
        options = {
            ...
            zoom: 0,
            beep: 'audio/beep.mp3',
            decoderWorker: 'content/DecoderWorker.js',
            brightness: 0,
    ...
    return {
        init: function(opt) {
            ...
            initialized = true;
            setEventListeners();
            DecodeWorker = new Worker(URL.createObjectURL(new Blob(["("+jsworker_function.toString()+")()"], {type: 'text/javascript'})));//options.decoderWorker);
            if (options.decodeQRCodeRate || options.decodeBarCodeRate) {
                setCallBack();
            }
            return this;
